...
module "cloud_run" {
  ...
  docker_repository_path = "${var.region}-docker.pkg.dev/${data.google_project.project.project_id}/${module.artifact_registry.docker_repository_name}"
  docker_service_account_email = module.artifact_registry.docker_service_account_email
  vpc_connector_id = module.network.cloud_run_vpc_access_connector_id
...
module "functions" {
  ...
  vpc_connector = module.network.cloud_run_vpc_access_connector_id
  region = var.gcp_project.region
}
